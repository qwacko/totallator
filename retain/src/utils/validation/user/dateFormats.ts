...
export const dayjsFormatter = (input: DBDateFormatTypes) => {
	return dateFormats.find((item) => item.dbValue === input)?.dayjsFormat || 'YYYYMMDD';
};

export const dateFormatter = (input: DBDateFormatTypes) => {
	return dateFormats.find((item) => item.dbValue === input)?.dateFnFormat || 'yyyyMMdd';
};

export const dateFormatOptions: { value: DBDateFormatTypes; label: string }[] = dateFormats.map(
	(item) => ({
		value: item.dbValue,
