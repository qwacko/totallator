...
export const buildSearchIDList = ({
	...
	const createSimpleListTo =
		data.createSimpleTransactions?.map((item) => item[key === 'accountId' ? 'toAccountId' : key]) ||
		[];
	...
	return [...upsertList, ...createSimpleListFrom, ...createSimpleListTo, ...createTransactionsList];
};
