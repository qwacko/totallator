	...
	get: protectedProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			const returnJournals = journals.map((journal) => {
				...
				const userIsAdmin =
					user.admin || accountGrouping.adminUsers.map((item) => item.id).includes(user.id);

				const otherJournals = transaction['journalEntries'].map((otherJournal) => {
					return {
						id: otherJournal.id,
				...
				return {
					...omit(pickedJournal, ['year', 'yearMonth', 'yearMonthDay', 'yearQuarter', 'yearWeek']),
	...
	updateJournals: protectedProcedure.input(updateJournalInput).mutation(async ({ ctx, input }) => {
		...
		const amountInOtherJournals = input.data.otherJournals
			? input.data.otherJournals
					...
					.reduce((prev, current) => (current !== undefined ? true : prev), false)
			: false;
		const amountInCoreJournal = input.data.amount !== undefined;
		const updateOtherAmounts = amountInCoreJournal ? !amountInOtherJournals : amountInOtherJournals;
	...
	cloneTransactions: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			await ctx.prisma.$transaction(async (prismaClient) => {
				const expandedTransactions: (typeof transactions)[] = new Array(input.cloneCount).fill(
					transactions
				);
