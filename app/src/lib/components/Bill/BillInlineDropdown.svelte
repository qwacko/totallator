<script lang="ts">
	import { BillsDropdownDocument } from '$lib/graphqlClient/generated';
	...
	type dataType = BillSelectInfo | undefined;
	...
	$: billsList = queryStore({
		client,
		query: BillsDropdownDocument,
	...
	$: scrollItems = $billsList?.data?.bills?.bills
		? $billsList.data.bills.bills.map((item) => ({
				type: 'data',
				onClick: () => {
	...
	<svelte:fragment slot="dropdownContent">
		<SelectableList
			...
			<svelte:fragment slot="fixed" let:data>
				<BillInlineDisplay value={data.data} />
			</svelte:fragment>
			<svelte:fragment slot="scroll" let:data
				><BillInlineDisplay value={data.data} /></svelte:fragment>
		</SelectableList>
	</svelte:fragment>
</TableDropdownItem>
