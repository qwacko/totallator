...
import type { JournalSortValidation } from '$lib/validation/journalEntries/getJournalValidation';

export const journalSortToOrderBy = (
	input: JournalSortValidation
): Prisma.Enumerable<Prisma.JournalEntryOrderByWithRelationAndSearchRelevanceInput> | undefined => {
	if (!input) {
		return [{ date: 'desc' }, { amount: 'desc' }, { createdAt: 'desc' }, { id: 'asc' }];
	}
