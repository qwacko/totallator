import type { Prisma, PrismaClient, AuthUser } from '@prisma/client';

import { dateRangeToDates } from '$lib/validation/journalEntries/dateRangeToDates';
import type { JournalFilterValidation } from '$lib/validation/journalEntries/getJournalValidation';
		...
		? await Promise.all(
				filters.map(async (filter) => {
					...
					if (filter.account) {
						...
						filter.accountId = {
							in: [...(filter?.accountId?.in ? filter.accountId.in : []), ...accountIds]
						};
					}
					return filter;
				})
...
export const journalsWithStats = async ({
	...
	const [allJournals, journalCountDefault, journalTotalDefault] = await Promise.all([
	...
	const startingTotal = startingTotalMaybeNull ? startingTotalMaybeNull.toNumber() : 0;
