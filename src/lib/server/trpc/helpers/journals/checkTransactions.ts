...
import { compareLinked } from '$lib/utils/compareLinked';
...
export const checkTransactions = async ({
	...
	transactions.map((transaction) => {
		...
		const hasAssetLiability = transaction.journalEntries.reduce(
			(prev, current) =>
				current.account.type === 'Asset' || current.account.type === 'Liability' ? true : prev,
			false
		);
		if (!hasAssetLiability) {
			throw new TRPCError({
				message: 'Transaction must have at least one account which is an asset or liability',
				code: 'INTERNAL_SERVER_ERROR'
			});
		}
		...
		if (firstJournal) {
			...
			if (linked) {
				transaction.journalEntries.map((journal) => {
					...
					if (!linkedMatched) {
						throw new TRPCError({
							message: "For a linked transaction, some linked properties don't match",
							code: 'BAD_REQUEST'
						});
					}
				});
