...
import type { UpdateJournalDataInputType } from '$lib/validation/journalEntries/updateJournalValidation';
...
export const updateSingleJournal = async ({
	...
	const dataWithComplete = data.complete ? { ...data, reconciled: true, dataChecked: true } : data;
		...
		if (transaction) {
			...
			if (total !== 0 && first) {
				const oldestTrans = transaction.journalEntries.reduce(
					(prev, current) => (current.updatedAt < prev?.updatedAt ? current : prev),
					first
				);
