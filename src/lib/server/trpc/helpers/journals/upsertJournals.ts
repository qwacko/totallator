...
import { removeUndefinedAndDuplicates } from '../arrayHelpers';
import type { UpsertJournalsValidationType } from '$lib/validation/journalEntries/upsertJournalValidation';
...
export const upsertJournals = async ({
	...
	const processedData: UpsertJournalsValidationType = internalData.map((journal) => {
	...
	const journalIds = removeUndefinedAndDuplicates(processedData.map((item) => item.id));
	...
	await Promise.all(
		inputJournalsToUpdate.map(async (journal) => {
			const currentJournal = journalsToUpdate.find((item) => item.id === journal.id);
			if (!currentJournal) {
				throw new TRPCError({
	...
	await Promise.all(
		transactionIds.map(async (transactionId) => {
			const journals = inputJournalsToCreate.filter((item) => item.transactionId === transactionId);
