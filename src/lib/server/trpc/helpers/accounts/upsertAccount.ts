...
import type { createAccountValidationType } from '$lib/validation/account/createAccountValidation';
import type { updateAccountDataValidationType } from '$lib/validation/account/updateAccountValidation';
import { basicStatusToDB, basicStatusToDBRequired } from '$lib/validation/basicStatusToDB';

import type { UpsertActions } from '../types';
import { createAccountGroupTitle, updateAccountGroupTitle } from './accountTitleGroupHandling';
import { defaultIncExp } from './defaultIncExp';

type UpsertAccountData = createAccountValidationType | updateAccountDataValidationType;
...
export const upsertAccount = async ({
				...
				data: {
					...
					...updateAccountGroupTitle({
						...
						accountGroupCombined:
							'accountGroupCombined' in data ? data.accountGroupCombined : undefined,
	...
	if (accountGroupingId && data.title) {
		...
		const { title, accountGroup, accountGroup2, accountGroup3, ...other } = data;
