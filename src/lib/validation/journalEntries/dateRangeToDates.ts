...
import type { UserReturn } from '$lib/server/trpc/routers/user';
...
export const dateRangeToDates = ({
	...
	user: UserReturn | AuthUser | undefined;
...
export const getDateRanges = ({
	...
	user: AuthUser | UserReturn | undefined;
}): {
	id: dateRangeType;
	title: string;
	...
	const now = new Date();
	const startOfFY = addMonths(startOfYear(subMonths(now, firstMonth - 1)), firstMonth - 1);
	const endOfFY = endOfMonth(addMonths(endOfYear(subMonths(now, firstMonth - 1)), firstMonth - 1));
