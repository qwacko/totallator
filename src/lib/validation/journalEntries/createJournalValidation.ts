...
import { compareLinked } from '../../utils/compareLinked';
...
export const createTransactionValidation = z
	...
	.refine(
		...
		{
			message: 'For linked journal entries, all values other than account and amount must be equal'
		}
	)
	.refine((data) => new Set(data.map((item) => item.accountGroupingId)).size === 1, {
		message: 'All journals for a transaction must be the same account grouping'
	})
