<script lang="ts">
	import { BudgetsDropdownDocument } from '$lib/graphqlClient/generated';
	...
	type dataType = BudgetSelectInfo | undefined;
	...
	$: budgetsList = queryStore({
		client,
		query: BudgetsDropdownDocument,
	...
	$: scrollItems = $budgetsList?.data?.budgets?.budgets
		? $budgetsList.data.budgets.budgets.map((item) => ({
				type: 'data',
				onClick: () => {
	...
	<svelte:fragment slot="dropdownContent">
		<SelectableList
			...
			<svelte:fragment slot="fixed" let:data>
				<BudgetInlineDisplay value={data.data} />
			</svelte:fragment>
			<svelte:fragment slot="scroll" let:data
				><BudgetInlineDisplay value={data.data} /></svelte:fragment>
		</SelectableList>
	</svelte:fragment>
</TableDropdownItem>
