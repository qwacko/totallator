<script lang="ts">
	import { TagsDropdownDocument } from '$lib/graphqlClient/generated';
	...
	import type { TagSelectInfo } from '../Table/TableTypes';
	import TagInlineDisplay from './TagInlineDisplay.svelte';

	const dispatch = createEventDispatcher();

	export let value: TagSelectInfo | undefined;
	...
	type dataType = TagSelectInfo | undefined;
	...
	$: scrollItems = $tagsList?.data?.tags?.tags
		? $tagsList.data.tags.tags.map((item) => ({
				type: 'data',
				onClick: () => {
	...
	<svelte:fragment slot="dropdownContent">
		<SelectableList
			...
			<svelte:fragment slot="fixed" let:data>
				<TagInlineDisplay value={data.data} />
			</svelte:fragment>
			<svelte:fragment slot="scroll" let:data
				><TagInlineDisplay value={data.data} /></svelte:fragment>
		</SelectableList>
	</svelte:fragment>
</TableDropdownItem>
