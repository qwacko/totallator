<script lang="ts">
	import {
		AccountsDropdownAllDocument,
		AccountsDropdownDocument
	...
	import AccountInlineDisplay from './AccountInlineDisplay.svelte';

	const dispatch = createEventDispatcher();

	export let value: AccountSelectInfo | undefined;
	export let accountGrouping: string | undefined;
	...
	type dataType = AccountSelectInfo | undefined;
			...
			fixedItems = [
				{ type: 'search' },
				{
					...
					data: {
						id: 'test',
						title: searchText || '',
						accountTitleCombined: 'Add New Expense Account',
						type: 'Asset'
					},
					onClick: () => {
						dispatch('update', searchText);
						closeDropdown();
					}
				}
			];
	...
	$: (value || searchText || includeNew) && updateFixedItems();

	//Query to get the items to populate the dropdown with
	const client = getContextClient();
	$: accountsList = queryStore({
		client,
		query: AccountsDropdownAllDocument,
		variables: {
			filter: {
				accountTitleCombined: { contains: searchTextDebounced },
				accountGroupingId: accountGrouping ? { equals: accountGrouping } : undefined
			}
		}
	});
	...
	$: scrollItems = $accountsList.data?.accounts.accounts
		? $accountsList.data.accounts.accounts.map((item) => ({
				type: 'data',
				onClick: () => {
	...
	<svelte:fragment slot="dropdownContent">
		<SelectableList
			...
			<svelte:fragment slot="fixed" let:data>
				<AccountInlineDisplay value={data.data} />
			</svelte:fragment>
			<svelte:fragment slot="scroll" let:data
				><AccountInlineDisplay value={data.data} /></svelte:fragment>
		</SelectableList>
	</svelte:fragment>
</TableDropdownItem>
